...
bool loadInput(string path_str) {
    ...
    //filesystem::path cwd = std::filesystem::current_path() / path_str;
    //cout<<cwd.string();
    //path_str = cwd.string();
...
void keyboardFunc(unsigned char key, int x, int y)
{
    ...
    case 'c':
        ...
        cout << "Smash C for no matter how many times you want to enjoy a disco object. xD" << endl;
        break;
    case 'r':
        rotation_angle += 5.0f;
        break;
...
//    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE, diffColors[0]);
    ...
    if (file_path.length() == 0) {
        cout << "EMPTY FILE PATH. Loading teapot." << endl;
        glutSolidTeapot(1.0);
    }
    else {
        bool load_designated_obj = loadInput(file_path);
        if (load_designated_obj == 0) {
            cout << "INVALID FILE PATH. Loading teapot." << endl;
            glutSolidTeapot(1.0);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    if (filename == 0) {
        cout << "CANNOT FIND FILE. Loading teapot." << endl;
    }
    else {
        file_path = filename;
